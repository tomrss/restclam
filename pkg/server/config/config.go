...
func loadLogConfig(v *viper.Viper) LogConfig {
	v.SetDefault("log.level", "debug") // TODO
...
func loadClamConfig(v *viper.Viper) ClamConfig {
	...
	v.SetDefault("clam.prewarmthSessions", 1)
	v.SetDefault("clam.maxIdleSessions", 20)
	...
	return ClamConfig{
		...
		PrewarmthSessions: v.GetInt("clam.prewarmthSessions"),
		MaxIdleSessions:   v.GetInt("clam.maxIdleSessions"),
...
//   - set an environment variable prefixed by "BPM_" with the name of the config
//     property uppercase with dots replaced with underscores (_).
//     Examples: BPM_DATABASE_USERNAME=admin, BPM_HELM_CHART_CACHE_ENABLED=true.
//
// The config file will be searched, in order, in this locations:
//   - /etc/besight-plugin-manager/config.yaml
//   - $HOME/.besight-plugin-manager/config.yaml
//   - ./config.yaml
