...
func (c *Clamd) Instream(r io.Reader) (*ScanResult, error) {
	...
	for {
		...
		if err != nil && err != io.EOF {
			return nil, fmt.Errorf("%w: error reading stream chunk: %w", ErrClamd, err)
		}
		if n == 0 {
			// end of read
			break
		...
		if _, err := c.conn.Write(buf); err != nil {
			return nil, fmt.Errorf("%w: error writing stream chunk: %w", ErrClamd, err)
		}
	...
	if _, err := c.conn.Write(buf); err != nil {
		return nil, fmt.Errorf("%w: error writing stream finalizer: %w", ErrClamd, err)
	}
...
func (c *Clamd) recvScanReply() (*ScanResult, error) {
	...
	if err != nil {
		return sr, fmt.Errorf("%w: unable to parse scan reply: %w", ErrClamd, err)
	}
...
func parseScanResult(statusLine string) (*ScanResult, error) {
	...
	if requestIDStr != "" {
		...
		if err != nil {
			return nil, fmt.Errorf("%w: non-integer request id: '%s'", ErrClamd, requestIDStr)
		}
